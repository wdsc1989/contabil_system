...
def init_database():
    ...
    # Cria configurações padrão para clientes existentes
    print("\nCriando configurações padrão de relatórios para clientes existentes...")
    db = SessionLocal()
    try:
        clients = db.query(Client).all()
        for client in clients:
            ReportConfigService.ensure_default_config(db, client.id)
        print(f"✅ Configurações padrão criadas para {len(clients)} cliente(s).")
